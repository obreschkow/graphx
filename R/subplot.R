...
subplot = function(xleft=NA, ybottom, xright, ytop) {
  ...
  if (is.numeric(xleft)) {
    ...
    .graphx.env$master.par = par(no.readonly = TRUE)
    ...
    if (xleft=='off') {
      ...
      # close subplot
      if (is.null(.graphx.env$master.par)) stop('subplot("off") called without an open subplot')
      return(.graphx.env$master.par)
